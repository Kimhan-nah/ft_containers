...
// SECTION vector_base : protected vector_base_common
...
class __vector_base : protected __vector_base_common<true> {
 ...
 protected:
  ...
  _LIBCPP_INLINE_VISIBILITY
  void __move_assign_alloc(__vector_base& __c) _NOEXCEPT_(
...
// SECTION vector_base(const allocator_type&);
template <class _Tp, class _Allocator>
inline _LIBCPP_INLINE_VISIBILITY __vector_base<_Tp, _Allocator>::__vector_base(
...
template <class _Tp, class _Allocator>
inline _LIBCPP_INLINE_VISIBILITY __vector_base<_Tp, _Allocator>::__vector_base(
    allocator_type&& __a) _NOEXCEPT : __begin_(nullptr),
                                      __end_(nullptr),
...
// SECTION vector : private vector_base
...
class _LIBCPP_TEMPLATE_VIS vector : private __vector_base<_Tp, _Allocator> {
 ...
 public:
  ...
  _LIBCPP_NODISCARD_AFTER_CXX17 _LIBCPP_INLINE_VISIBILITY bool empty()
      const _NOEXCEPT {
 ...
 private:
  ...
  _LIBCPP_INLINE_VISIBILITY void __invalidate_iterators_past(
      pointer __new_last);
  ...
  template <class... _Args>
  _LIBCPP_INLINE_VISIBILITY inline void __emplace_back_slow_path(
      _Args&&... __args);
#else
  template <class _Up>
  _LIBCPP_INLINE_VISIBILITY inline void __push_back_slow_path(_Up& __x);
  ...
  // allocators may not meet the AddressSanitizer alignment constraints. See the
  // documentation for __sanitizer_annotate_contiguous_container for more
  // details.
...
void vector<_Tp, _Allocator>::__swap_out_circular_buffer(
    __split_buffer<value_type, allocator_type&>& __v) {
  __annotate_delete();
  _VSTD::__construct_backward_with_exception_guarantees(
...
vector<_Tp, _Allocator>::__construct_at_end(_ForwardIterator __first,
                                            ...
                                            size_type __n) {
  _ConstructTransaction __tx(*this, __n);
  _VSTD::__construct_range_forward(this->__alloc(), __first, __last,
                                   __tx.__pos_);
}
// !SECTION enable_if<>::type
...
// SECTION vector(n, x, allocator_type& a);
template <class _Tp, class _Allocator>
vector<_Tp, _Allocator>::vector(size_type __n, const value_type& __x,
                                const allocator_type& __a)
    : __base(__a) {
...
// SECTION vector(InputIterator, enbale_if<>);
template <class _Tp, class _Allocator>
template <class _InputIterator>
vector<_Tp, _Allocator>::vector(
  ...
  __get_db()->__insert_c(this);
#endif
  for (; __first != __last; ++__first) __emplace_back(*__first);
}
// !SECTION
...
// SECTION vector(ForwardIterator, enable_if<>);
template <class _Tp, class _Allocator>
template <class _ForwardIterator>
vector<_Tp, _Allocator>::vector(
    _ForwardIterator __first,
...
template <class _Tp, class _Allocator>
inline _LIBCPP_INLINE_VISIBILITY vector<_Tp, _Allocator>::vector(
    vector&& __x, const __identity_t<allocator_type>& __a)
    : __base(__a) {
#if _LIBCPP_DEBUG_LEVEL == 2
  __get_db()->__insert_c(this);
#endif
...
template <class _Tp, class _Allocator>
inline _LIBCPP_INLINE_VISIBILITY void vector<_Tp, _Allocator>::push_back(
    const_reference __x) {
...
template <class _Allocator>
vector<bool, _Allocator>& vector<bool, _Allocator>::operator=(
    const vector& __v) {
  if (this != &__v) {
...
template <class _Allocator>
typename vector<bool, _Allocator>::reference vector<bool, _Allocator>::at(
    size_type __n) {
  if (__n >= size()) this->__throw_out_of_range();
  return (*this)[__n];
}
...
template <class _Allocator>
typename vector<bool, _Allocator>::const_reference vector<bool, _Allocator>::at(
    size_type __n) const {
  if (__n >= size()) this->__throw_out_of_range();
  return (*this)[__n];
}
...
template <class _Allocator>
typename vector<bool, _Allocator>::iterator vector<bool, _Allocator>::insert(
...
template <class _Allocator>
typename vector<bool, _Allocator>::iterator vector<bool, _Allocator>::erase(
    const_iterator __first, const_iterator __last) {
...
inline _LIBCPP_INLINE_VISIBILITY bool operator==(
    const vector<_Tp, _Allocator>& __x, const vector<_Tp, _Allocator>& __y) {
